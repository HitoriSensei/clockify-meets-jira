...
import { UpdatedPayload } from "./payload/updated";
...
export function isCreated(payload: WebhookPayload): payload is CreatedPayload {
  return (
    ...
    "action" in payload.metadata &&
    payload.metadata.action === "created"
  );
}
...
export function isUpdated(payload: WebhookPayload): payload is UpdatedPayload {
  return (
    typeof payload.metadata === "object" &&
    "action" in payload.metadata &&
    payload.metadata.action === "updated"
  );
}
...
export function isDeleted(payload: WebhookPayload): payload is UpdatedPayload {
  return (
    isUpdated(payload) &&
    payload.metadata.request_body.includes('"/deleted_at"')
  );
}

export function shouldIgnore(payload: WebhookPayload): string | false {
  return (
    (isPing(payload) && "Ping received") ||
    (isCreated(payload) &&
      !payload.payload.stop &&
      "Created entry without stop time") ||
    (isUpdated(payload) &&
      !payload.payload.stop &&
      "Updated entry without stop time") ||
